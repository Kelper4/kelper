...
create table sensor(
	idSensor 			int primary key not null,		-- Identificador único de cada sensor.
...
insert into sensor (idSensor, status, createdAt) values
...
create table leitura(
    ...
    intensidade 		decimal(10,2), 					-- Valor para armazenar todos os valores obtidos pelos sensores em lúmens.
    createdAt 			datetime						-- Data de quando a leitura foi feita, repare que não tem updatedAt(atualizado em...) pois as leituras não podem ser alteradas/atualizadas.
);
...
create table alerta(
    ...
    intensidade			decimal(10,2),					-- Média de luminosidade recebida pelo viveiro.
    nivelRisco			varchar(50), 					-- Armazenado em (Baixo, Médio e Grave).
    analise				varchar(255),					-- Descrição sobre o alerta.
    createdAt			datetime,					-- Data de quando o alerta foi feito, repare que não tem updatedAt(atualizado em...) pois os alertas não podem ser alterados/atualizados.
    
    constraint chk_nivelRisco check(nivelRisco in ("Baixo", "Médio", "Grave"))
);

-- 5. Populando Alertas
insert into alerta (idAlerta, intensidade, nivelRisco, analise, createdAt) values
(1, 150.00, 'Grave', 'Baixa luminosidade extrema detectada. Risco de bloom de algas ou alta turbidez.', '2026-03-02 13:05:00'),
